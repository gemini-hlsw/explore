...
case class AppConfig(
  ...
  todoURL: Url = Url.parse("https://cors-anywhere.herokuapp.com/https://todo-mongo-graphql-server.herokuapp.com/"),
  pollURL: Url = Url.parse("wss://realtime-poll.demo.hasura.app/v1/graphql")
)

class AppState[F[_] : ConcurrentEffect : Timer](config: AppConfig, val rootModel: Model[F, RootModel]) {
  object Clients {
  ...
  object Actions {
    ...
    lazy val todoList: TodoListActions[F]  = new TodoListActionsInterpreter[F](Views.todoList)(Clients.todo)
...
object AppStateIO {
  ...
  private val value: Ref[SyncIO, Option[AppState[IO]]] = Ref.unsafe[SyncIO, Option[AppState[IO]]](None)

  def init(config: AppConfig): SyncIO[Unit] =
    value.access.flatMap{ case (oldValue, set) =>
      val multipleCallsError = SyncIO.raiseError[Unit](new Exception("Multiple calls to AppState init."))
