object Versions {
  val boopickle              = "1.5.0"
  val cats                   = "2.13.0"
  val catsEffect             = "3.6.1"
  val catsRetry              = "3.1.3"
  val catsTime               = "0.5.1"
  val circe                  = "0.14.13"
  val circeGolden            = "0.3.0"
  val coulomb                = "0.8.0"
  val clue                   = "0.43.1"
  val crystal                = "0.47.6"
  val discipline             = "1.7.0"
  val disciplineMUnit        = "2.0.0"
  val fs2                    = "3.12.0"
  val fs2Data                = "1.11.0"
  val fs2Dom                 = "0.3.0-M1"
  val geminiLocales          = "0.7.0"
  val http4s                 = "0.23.30"
  val http4sDom              = "0.2.11"
  val kittens                = "3.5.0"
  val log4Cats               = "2.7.0"
  val log4CatsLogLevel       = "0.3.1"
  val lucumaBC               = "0.4.0"
  val lucumaCore             = "0.124.0"
  val lucumaITC              = "0.34.2"
  val lucumaReact            = "0.82.0"
  val lucumaRefined          = "0.1.4"
  val lucumaSchemas          = "0.131-8614800-SNAPSHOT"
  val lucumaOdbSchema        = "0.19.3-51-a452ae3-20250421T210726Z-SNAPSHOT"
  val lucumaSSO              = "0.8.10"
  val lucumaUI               = "0.139.1-3-df1d2cb-20250422T221305Z-SNAPSHOT"
  val monocle                = "3.3.0"
  val mouse                  = "1.3.2"
  val mUnit                  = "1.1.0"
  val mUnitScalacheck        = "1.1.0"
  val mUnitCatsEffect        = "2.1.0"
  val refinedAlgebra         = "0.1.1"
  val sbtBuildInfo           = "0.13.1"
  val sbtLucuma              = "0.12.10"
  val scalaCollectionContrib = "0.4.0"
  val scalaJsDom             = "2.8.0"
  val scalaJsReact           = "3.0.0-beta12"
}
