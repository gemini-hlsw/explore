object Versions {
  val boopickle              = "1.4.0"
  val cats                   = "2.12.0"
  val catsEffect             = "3.5.4"
  val catsRetry              = "3.1.3"
  val catsTime               = "0.5.1"
  val circe                  = "0.14.7"
  val circeGolden            = "0.3.0"
  val coulomb                = "0.8.0"
  val clue                   = "0.35.2"
  val crystal                = "0.39.1"
  val discipline             = "1.7.0"
  val disciplineMUnit        = "2.0.0"
  val fs2                    = "3.10.2"
  val fs2Data                = "1.11.0"
  val fs2Dom                 = "0.3.0-M1"
  val geminiLocales          = "0.7.0"
  val http4s                 = "0.23.27"
  val http4sDom              = "0.2.11"
  val kittens                = "3.3.0"
  val log4Cats               = "2.7.0"
  val log4CatsLogLevel       = "0.3.1"
  val lucumaBC               = "0.4.0"
  val lucumaCore             = "0.98-8499bb5-SNAPSHOT"
  val lucumaCatalog          = "0.46.1-6-f685d2e-20240524T213355Z-SNAPSHOT"
  val lucumaReact            = "0.64.0"
  val lucumaRefined          = "0.1.3"
  val lucumaSchemas          = "0.86-8313222-20240524T205532Z-SNAPSHOT"
  val lucumaOdbSchema        = "0.11.7-62-ead8cef-20240524T202318Z-SNAPSHOT"
  val lucumaSSO              = "0.6.17"
  val lucumaUI               = "0.103.1"
  val lucumaITC              = "0.21.9-14-3813291-20240524T221100Z-SNAPSHOT"
  val monocle                = "3.2.0"
  val mouse                  = "1.2.3"
  val mUnit                  = "1.0.0"
  val mUnitCatsEffect        = "2.0.0"
  val reactAladin            = "0.32.0"
  val refinedAlgebra         = "0.1.1"
  val sbtBuildInfo           = "0.12.0"
  val sbtLucuma              = "0.11.15"
  val scalaCollectionContrib = "0.3.0"
  val scalaJsDom             = "2.8.0"
  val scalaJsReact           = "3.0.0-beta3"
  val webAppUtil             = "2.0.0-RC12"
}
