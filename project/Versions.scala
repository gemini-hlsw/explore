object Versions {
  val boopickle              = "1.4.0"
  val cats                   = "2.10.0"
  val catsEffect             = "3.5.4"
  val catsRetry              = "3.1.0"
  val catsTime               = "0.5.1"
  val circe                  = "0.14.6"
  val circeGolden            = "0.3.0"
  val coulomb                = "0.8.0"
  val clue                   = "0.35.0"
  val crystal                = "0.37.2-17-080c601-20240306T185543Z-SNAPSHOT"
  val discipline             = "1.5.1"
  val disciplineMUnit        = "1.0.9"
  val fs2                    = "3.9.4"
  val fs2Data                = "1.10.0"
  val fs2Dom                 = "0.3.0-M1"
  val geminiLocales          = "0.7.0"
  val http4s                 = "0.23.26"
  val http4sDom              = "0.2.11"
  val kittens                = "3.2.0"
  val log4Cats               = "2.6.0"
  val log4CatsLogLevel       = "0.3.1"
  val lucumaBC               = "0.4.0"
  val lucumaCore             = "0.93.0"
  val lucumaCatalog          = "0.44.7"
  val lucumaReact            = "0.51.0-2-6376fce-20240306T184129Z-SNAPSHOT"
  val lucumaRefined          = "0.1.3"
  val lucumaSchemas          = "0.75.0"
  val lucumaOdbSchema        = "0.11.1"
  val lucumaSSO              = "0.6.13"
  val lucumaUI               = "0.94.2-2-a0148bf-20240306T190205Z-SNAPSHOT"
  val lucumaITC              = "0.21.5"
  val monocle                = "3.2.0"
  val mouse                  = "1.2.3"
  val mUnit                  = "0.7.29"
  val mUnitCatsEffect        = "1.0.7"
  val pprint                 = "0.8.1"
  val reactAladin            = "0.31.2"
  val refinedAlgebra         = "0.1.1"
  val sbtBuildInfo           = "0.11.0"
  val sbtLucuma              = "0.11.13"
  val scalaCollectionContrib = "0.3.0"
  val scalaJsDom             = "2.8.0"
  val scalaJsReact           = "3.0.0-beta3"
  val webAppUtil             = "2.0.0-RC12"
}
