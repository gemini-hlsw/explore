object Versions {
  val boopickle              = "1.5.0"
  val cats                   = "2.12.0"
  val catsEffect             = "3.5.4"
  val catsRetry              = "3.1.3"
  val catsTime               = "0.5.1"
  val circe                  = "0.14.9"
  val circeGolden            = "0.3.0"
  val coulomb                = "0.8.0"
  val clue                   = "0.40.0"
  val crystal                = "0.42.0"
  val discipline             = "1.7.0"
  val disciplineMUnit        = "2.0.0"
  val fs2                    = "3.10.2"
  val fs2Data                = "1.11.0"
  val fs2Dom                 = "0.3.0-M1"
  val geminiLocales          = "0.7.0"
  val http4s                 = "0.23.27"
  val http4sDom              = "0.2.11"
  val kittens                = "3.4.0"
  val log4Cats               = "2.7.0"
  val log4CatsLogLevel       = "0.3.1"
  val lucumaBC               = "0.4.0"
  val lucumaCore             = "0.103.0"
  val lucumaCatalog          = "0.47.4"
  val lucumaReact            = "0.71-94bd0e5-SNAPSHOT"
  val lucumaRefined          = "0.1.3"
  val lucumaSchemas          = "0.96.0"
  val lucumaOdbSchema        = "0.12.0"
  val lucumaSSO              = "0.6.22"
  val lucumaUI               = "0.114.0-10-c1751b5-20240821T190720Z-SNAPSHOT"
  val lucumaITC              = "0.21.18"
  val monocle                = "3.3.0"
  val mouse                  = "1.3.2"
  val mUnit                  = "1.0.0"
  val mUnitCatsEffect        = "2.0.0"
  val reactAladin            = "0.32.2"
  val refinedAlgebra         = "0.1.1"
  val sbtBuildInfo           = "0.12.0"
  val sbtLucuma              = "0.11.17"
  val scalaCollectionContrib = "0.3.0"
  val scalaJsDom             = "2.8.0"
  val scalaJsReact           = "3.0.0-beta6"
  val webAppUtil             = "2.0.0-RC12"
}
