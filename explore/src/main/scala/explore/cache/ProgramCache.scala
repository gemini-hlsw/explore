    ...
    Resource.make(Channel.unbounded[IO, Unit])(_.close.void).flatMap { programUpdateChannel =>
      try {
        ...
        val obsTimesUpdates: Pipe[
          ...
          _.observationEdit.value.map(_.id),
          data =>
            data.observationEdit.value
              .map(_.id)
              .map(updateObservationExecution)
              .getOrElse(IO(identity[ProgramSummaries])) <* queryProgramTimes
        )
          ...
          ObsQueriesGQL.ProgramObservationsDelta
            ...
            .map:
              _.broadcastThrough(
                _.map { data =>
                  // println(data);
                  modifyObservations(data.observationEdit)
                },
                obsTimesUpdates
              )
