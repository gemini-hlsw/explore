  ...
  override final def run: IO[Unit] = {
    ...
    Dispatcher[IO].allocated
      ...
      .flatMap { param =>
        ...
        def pageUrl(tab: AppTab, focusedObs: Option[FocusedObs], focusedTarget: Option[Target.Id])
          : String =
          routerCtl.urlFor(RootModelRouting.getPage(tab, focusedObs, focusedTarget)).value

        def setPage(tab: AppTab, focusedObs: Option[FocusedObs], focusedTarget: Option[Target.Id]) =
