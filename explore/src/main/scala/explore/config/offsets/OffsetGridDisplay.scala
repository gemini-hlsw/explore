...
object OffsetGridDisplay {
  ...
  private def calculateLinesSeparation(
    ...
    val candidates =
      List(1.0, 2.0, 5.0, 10.0, 20.0, 25.0, 50.0, 100.0, 150.0, 200.0, 250.0, 300.0, 500.0)
  ...
  private def offsetArrows(
    ...
    def arrow(from: OffsetValues, to: OffsetValues, idx: Int) =
      ...
      if (length > 0) {
        val scaleFactor     =
          if (offsetValues.length > 50) 0.7 else if (offsetValues.length > 20) 0.8 else 1.0
    ...
    <.g(
      css,
      offsetValues
        .zip(offsetValues.drop(1))
        .zipWithIndex
        .map { case ((from, to), idx) =>
          arrow(from, to, idx)
        }
        .toTagMod
    )
