 ...
 */
inline def useCallback[A](callback: A)(using UseCallbackArg[A]): HookResult[Reusable[A]] =
 ...
 *   https://reactjs.org/docs/hooks-reference.html#usecallback
 */
inline def useCallbackWithDeps[D: Reusability, A](deps: => D)(callback: D => A)(using
  UseCallbackArg[A]
...
inline def useDebugValue(desc: => Any): HookResult[Unit] =
  HookResult:
    UseDebugValue.unsafeCreate(desc)
 ...
 */
inline def useEffect[A](effect: A)(using UseEffectArg[A]): HookResult[Unit] =
  HookResult:
    UseEffect.unsafeCreate(effect)
 ...
 *   https://reactjs.org/docs/hooks-reference.html#useeffect
 */
inline def useEffectOnMount[A](effect: A)(using UseEffectArg[A]): HookResult[Unit] =
  HookResult:
 ...
 *   https://reactjs.org/docs/hooks-reference.html#useeffect
 */
inline def useEffectWithDeps[D: Reusability, A](deps: => D)(effect: D => A)(using
  UseEffectArg[A]
): HookResult[Unit] =
 ...
 */
inline def useLayoutEffect[A](effect: A)(using UseEffectArg[A]): HookResult[Unit] =
  HookResult:
    UseEffect.unsafeCreateLayout(effect)
 ...
 *   https://reactjs.org/docs/hooks-reference.html#useLayoutEffect
 */
inline def useLayoutEffectOnMount[A](effect: A)(using UseEffectArg[A]): HookResult[Unit] =
  HookResult:
    UseEffect.unsafeCreateLayoutOnMount(effect)
 ...
 *   https://reactjs.org/docs/hooks-reference.html#useLayoutEffect
 */
inline def useLayoutEffectWithDeps[D: Reusability, A](deps: => D)(effect: D => A)(using
  UseEffectArg[A]
): HookResult[Unit] =
 ...
 */
inline def useReducer[S, A](
  reducer:      (S, A) => S,
  initialState: => S
): HookResult[UseReducer[S, A]] =
  HookResult:
    UseReducer.unsafeCreate(reducer, initialState)
...
inline def useRefToJsComponent[F[_], A[_], P1, S1, CT1[-p, +u] <: CtorType[p,
  ...
  HookResult:
    UseRef.unsafeCreateToJsComponent(a)

/** Create a mutable ref that will persist for the full lifetime of the component. */
inline def useRef[A](initialValue: => A): HookResult[UseRef[A]] =
  HookResult:
    UseRef.unsafeCreate(initialValue)
...
inline def useStateWithReuseBy[S: ClassTag: Reusability](
  ...
  HookResult:
    UseStateWithReuse.unsafeCreate(initialState)

// useForceUpdate ??? it's custom but in scalajs-react
