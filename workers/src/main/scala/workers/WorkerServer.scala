...
trait WorkerServer[F[_]: Async, T: Pickler](using Monoid[F[Unit]]):
  ...
  // Provides an oppotunity to plug in additional behaviour, like caching.
  protected def handlerInternal(
    self: DedicatedWorkerGlobalScope
  ): Logger[F] ?=> F[Invocation => F[Unit]] = handler
  ...
  protected def runInternal(dispatcher: Dispatcher[F])(using Logger[F]): F[Unit] =
    for {
      ...
      handlerFn    <- handlerInternal(self)
