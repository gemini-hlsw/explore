...
object ErrorMsg {
  ...
  implicit def univEq: Eq[ErrorMsg] =
    Eq.by(_.value)
  ...
  object ClientSide {
    ...
    def noCompatibleServer    = ErrorMsg(
      "Failed to find a compatible server. Please try again, or try reloading the page."
    )
    def serverCallTimeout     = ErrorMsg(
      "Server didn't respond. Please check your internet connectivity."
    )
    def serverProtocolIsNewer = ErrorMsg(
      "Our servers have been upgraded to a newer version. Please reload this page and try again."
    )
  }
